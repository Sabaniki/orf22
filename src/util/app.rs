...
pub fn get_arg() -> Result<(String, String, String, String), anyhow::Error> {
    let app = create_app();
    let net_name = app.value_of("net_name")
        .with_context(||"could not get the arg [net_name]")?;
    ...
    Ok((net_name.to_string(), interface_name.to_string(), ipv4_range.to_string(), ipv6_range.to_string()))
}
